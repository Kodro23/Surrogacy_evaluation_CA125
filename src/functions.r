...
get_monthly_slopes <- function(time_grid, y_ctrl, y_trt, months = 1:6) {
  ...
  sapply(months, function(m) {
    ...
    target_time <- m * 30
    idx <- which.min(abs(time_grid - target_time))

    if (idx >= length(time_grid)) return(NA)

    dy_c <- (y_ctrl$pred[idx + 1] - y_ctrl$pred[idx]) /
            (time_grid[idx + 1] - time_grid[idx])

    dy_t <- (y_trt$pred[idx + 1] - y_trt$pred[idx]) /
            (time_grid[idx + 1] - time_grid[idx])
...
get_monthly_pct_reduction <- function(time_grid, y_ctrl, y_trt, months = 1:6){
  ...
  sapply(months, function(m){
    ...
    pct_ctrl <-
      (baseline_ctrl - y_ctrl$pred[idx,1]) /
      baseline_ctrl

    pct_trt <-
      (baseline_trt - y_trt$pred[idx,1]) /
      baseline_trt
